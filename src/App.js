...
//Check if word is in wordDictionary array and return true or false
...
function checkEnter(e) {
  if (e.keyCode === 13) {
    playGame(document.getElementById("word1").value);
  }
}

//Display the number of tries used 
function displayTries() {
  document.getElementById("tries").innerHTML = "Try Number: " + tries;
}

//Play the game
function playGame(word) {
  if (word.length == 5 && checkWord(word)) {
    if (tries < 6 && won != true) {
      //Compare words
      ...
      alert("You lost the word was " + word2);
    }
  } else {
    //Display a modal saying invalid
    alert("Word is not 5 letter or not in dictionary");
  }
}
...
function displayWord(word, result) {
  ...
  for (let i = (tries * 5); i < 5 + (tries * 5); i++) {
    //Display the word in the box and add animation
    ...
    <div className="App">
      <div className="App-body">
        ...
        <div className="App-body-center">
          <div>
            {/*Make a text box to take a word and then run it against the compare word function and check enter */}
            <input type="text" autoComplete="off" id="word1" style={{ height: "30px", width: "200px", fontSize: 30, textAlign: "center" }} placeholder="Enter a word" onKeyUp={checkEnter} />
          </div>
          <button style={{ height: "50px", margin: "5px" }} onClick={() => { playGame(document.getElementById("word1").value) }}>Compare</button>
          {/* Button to reset the game */}
          <button style={{ height: "50px", margin: "5px" }} onClick={() => { resetGame() }}>Reset Game</button>
        </div>
        <div>
          <h2 id="tries">Try Number: 0</h2>
          <p style={{ color: "white" }}>
            Made by Sam Kuhbander
          </p>
        </div>
      </div>
    </div>
